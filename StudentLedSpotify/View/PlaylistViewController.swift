		 ...
		 if let imageUrl = URL(string: self.playlist?.image ?? ""), let userUrl = URL(string: self.playlist?.profile_picture ?? "") {
			self.songFetcher.loadImage(from: imageUrl) { (image) in
			   self.songFetcher.loadImage(from: userUrl) { (img) in
				  DispatchQueue.main.async{
					 headerView.configureView(profileImageView:img!,playlistImageView: image!, userName: self.playlist?.username ?? "", playlistName: self.playlist?.playlist ?? "", songsCount: " \(self.playlist?.n_tracks ?? 0) songs, \(self.playlist?.duration ?? "")")
				  }
			   }
			}
		 }
